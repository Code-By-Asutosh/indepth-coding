...
export const HIBERNATE_ENTITY_LIFECYCLE: ConceptContent = {
  ...
  topicType: "framework",
  simpleIntuition: "You fetch a User inside a transaction, the transaction commits, and later, outside any transaction, you change the user's name and call save() again. It works. But if you had skipped that second save() call and just changed the field, nothing would have happened at all. Same object, same field, wildly different outcome. Why?",
  formalMeaning: "Every entity object is in exactly one of four states at any moment, and only one of those states makes Hibernate automatically track and save your changes.",
  whyItExists: "A Hibernate entity is not always \"live.\" Whether changing a field actually reaches the database depends entirely on which lifecycle state that specific object is in at that specific moment, and that state is invisible just by looking at the object itself.",
  ...
  complexityAndTradeoffs: [
    ...
    "This exact confusion, \"why did my change not save,\" is one of the most common Hibernate support questions, precisely because the failure mode is silent rather than an exception.",
    "Keep entities managed for the full duration of a use case (do everything inside one @Transactional method): use it when most typical request handling, where you load, modify, and the transaction commits all within one method call. Avoid it when long running processes or multi step wizards where holding a transaction open the whole time would hold database locks too long.",
    "Explicitly merge detached entities (repository.save()): use it when data arriving from outside the current transaction, like a deserialized request body or an object passed across service boundaries. Avoid it when data that is already managed, calling merge on an already managed entity is redundant.",
    "DTOs instead of exposing entities directly: use it when you want to avoid lifecycle confusion entirely by never letting entity objects leave the transactional boundary in the first place. Avoid it when very small internal tools where the extra mapping layer is not worth the effort."
  ],
  commonMistakes: [
    "Modifying an entity object outside of any transaction and expecting the change to be saved automatically, the way it would be inside one. It genuinely does work that way while the entity is managed, so the mental model \"just change the field and it saves\" is not wrong, it is just incomplete. It only holds while the persistence context is still open. Fix: Either keep the modification inside the same transactional boundary as the load, or explicitly call save()/merge() when working with a detached entity."
  ],
  interviewPerspective: "A common way this gets tested: \"You load an Order inside a @Transactional method and return it. The caller, outside any transaction, sets order.setTotal(new BigDecimal(\"99.99\")). Does this change reach the database?\" No, not unless the caller explicitly calls save()/merge() afterward. The Order became detached the moment the transactional method returned, so Hibernate is no longer tracking field changes on it.",
  triggerSentence: "A Hibernate entity only saves itself automatically while it is Managed. Everywhere else, you have to ask for it explicitly."
};
