...
export const JPA: ConceptContent = {
  ...
  title: "JPA",
  topicType: "framework",
  simpleIntuition: "You call orderRepository.save(order) and never write a single line of SQL, yet an INSERT statement genuinely runs against your database. Somewhere between your method call and the database, Java objects turned into SQL. What is actually in between?",
  formalMeaning: "JPA is a specification, a set of interfaces and annotations, describing how Java objects map to database tables. Hibernate is the most common actual implementation of that specification.",
  whyItExists: "Writing raw JDBC for every entity in a real application means hand writing repetitive INSERT, UPDATE, SELECT statements and manually copying ResultSet columns into object fields, for every single table, over and over. JPA exists to describe that mapping once, declaratively, and let a provider generate the repetitive SQL for you.",
  ...
  complexityAndTradeoffs: [
    ...
    "Teams adopting JPA over raw JDBC commonly report dramatically less boilerplate per entity, at the cost of needing to genuinely understand what the provider is generating underneath, which is exactly why concepts like the N+1 Problem matter.",
    "JPA / Hibernate: use it when the default choice for most applications with a real object model and evolving schema, where developer productivity on CRUD and relationships matters. Avoid it when extremely performance sensitive read paths where hand tuned SQL and a lightweight mapper give more predictable query plans.",
    "MyBatis / jOOQ: use it when you want to write real SQL yourself but still get typed results mapped to objects, without an ORM's automatic dirty checking and lazy loading behavior. Avoid it when you want relationships and entity state managed for you automatically, which is exactly what a full ORM provides.",
    "Raw JDBC: use it when very simple applications, libraries, or migration tools with a handful of queries total. Avoid it when a real application with a meaningful, evolving domain model."
  ],
  commonMistakes: [
    "Treating JPA entities like plain data objects and being surprised when modifying a field outside a transaction does nothing, or modifying it inside one silently triggers an UPDATE you did not expect. A JPA entity is only \"just data\" when it is detached. While it is managed, inside an active persistence context, every field change is tracked and will be written to the database at commit time, whether or not you explicitly called save(). Fix: Understand entity lifecycle states (see Hibernate Entity Lifecycle) so you know exactly when a change to an object will, or will not, reach the database."
  ],
  interviewPerspective: "A common way this gets tested: \"Inside a @Transactional method, you load an Order, change its status field, and never call save() or update(). Does the change reach the database?\" Yes, assuming the Order is currently managed by the persistence context. JPA's dirty checking detects the field change and issues an UPDATE automatically when the transaction commits, with no explicit save call required.",
  triggerSentence: "JPA is the standard, Hibernate is one very good implementation of it, and dirty checking is the reason your saved changes sometimes need no explicit save call at all."
};
