...
export const CONNECTION_POOLING: ConceptContent = {
  ...
  topicType: "framework",
  simpleIntuition: "Your API opens a fresh database connection for every single request, and each request quietly costs an extra 20 to 50 milliseconds before your query even runs. Multiply that by a thousand requests a minute and you have built a performance problem that never shows up as a single slow line of code.",
  formalMeaning: "A connection pool opens a batch of database connections once, keeps them alive, and hands them out to whichever request needs one, returning them to the pool instead of closing them.",
  whyItExists: "Opening a new database connection means a TCP handshake, then authentication, then the database allocating memory and a session for you, then eventually tearing all of that down when you close it. Doing this per request is like hiring and training a brand new employee just to answer one phone call, then firing them immediately after.",
  ...
  complexityAndTradeoffs: [
    ...
    "Adding a connection pool to a service that previously opened raw connections per request commonly removes the single largest fixed latency cost on every database backed endpoint.",
    "HikariCP: use it when the default, modern choice for almost every JVM application. It is what Spring Boot uses out of the box. Avoid it when rare cases needing a specific feature only another pool implementation provides.",
    "Apache Commons DBCP / Tomcat JDBC pool: use it when legacy applications already built around one of these, or environments with existing operational tooling around them. Avoid it when new projects. HikariCP is generally faster and simpler to reason about.",
    "No pooling, raw DriverManager connections: use it when a short lived script or a one off migration tool that opens a handful of connections total. Avoid it when any long running service handling concurrent requests."
  ],
  commonMistakes: [
    "Setting the pool's maximum size far higher than the database server's own max connections limit, assuming bigger is always safer. A single application instance with an oversized pool can, under load, consume most or all of the database's connection limit, starving every OTHER service or replica connecting to the same database, causing a cluster wide outage instead of just a slow single service. Fix: Size the pool based on the database's actual connection limit divided across every service instance and replica that connects to it, not just based on one service's own expected concurrency."
  ],
  interviewPerspective: "A common way this gets tested: \"When your code calls connection.close() on a pooled connection, what actually happens to the underlying TCP connection to the database?\" Nothing happens to it. The pool intercepts close() and returns the connection to its idle set so another request can reuse the same already open TCP connection. The real connection is only actually closed when the pool itself shuts down or decides to recycle it.",
  triggerSentence: "A connection pool turns the expensive part of talking to a database, opening the connection, into a one time cost instead of a per request cost."
};
